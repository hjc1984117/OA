var phoneSysListViewModel = new function () {
    ...
    self_.listPhoneSys = function (data) {
        ycoa.ajaxLoadGet("/api/sale_soft/phone_sys.php", data, function (results) {
            self_.phoneSysList.removeAll();
            $.each(results.list, function (index, phoneSys) {
                phoneSys.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("3031002");
                phoneSys.edit = ycoa.SESSION.PERMIT.hasPagePermitButton("3031003");
                phoneSys.show = ycoa.SESSION.PERMIT.hasPagePermitButton("3031004");
    ...
    self_.delPhoneSys = function (phoneSys) {
        ycoa.UI.messageBox.confirm('确认删除？', function (del) {
            if (del) {
                phoneSys.action = 2;
                ycoa.ajaxLoadPost("/api/sale_soft/phone_sys.php", JSON.stringify(phoneSys), function (result) {
                    if (result.code == 0) {
    ...
    self_.doEditPhoneSys = function (phoneSys) {
        ...
        data.action = 3;
        ycoa.ajaxLoadPost("/api/sale_soft/phone_sys.php", JSON.stringify(data), function (result) {
            if (result.code == 0) {
                ycoa.UI.toast.success(result.msg);
                reLoadData({action: 1});
            } else {
...
$(function () {
    ...
    $("#btn_toexcel_primary").live("click", function () {
        ...
        if (start_time || end_time) {
            location.href = '/api/sale_soft/phone_sys.php?start_time=' + start_time + '&end_time=' + end_time + '&action=11';
        }
    });
