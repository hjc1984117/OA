    ...
    self_.listPhysica = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale_soft/physica.php", data, function (results) {
            ...
            $.each(results.list, function (index, physica) {
                ...
                physica.dele = ycoa.SESSION.PERMIT.hasPagePermitButton('3060202') && ( ycoa.user.userid() === physica.customer_id || physica.is_manager === true);
                physica.edit = ycoa.SESSION.PERMIT.hasPagePermitButton('3060203') && ( (physica.isTe === 0 && ycoa.user.userid() === physica.customer_id && physica.is_edit === 0) || physica.is_manager === true);//售后一般人员在审核后没有修改权限;
                physica.show = ycoa.SESSION.PERMIT.hasPagePermitButton('3060204');
                physica.te = ycoa.SESSION.PERMIT.hasPagePermitButton('3060206') && physica.isTe === 0 && physica.is_manager === true;
...
//    });
    ...
    $("#show_ranking").click(function () {
        ycoa.ajaxLoadGet("/api/second_sale_soft/platform.php", {action: 11, time_unit: 1, tab: $("#auto_tab").val()}, function (result) {
            ...
            html += "</ul>";
            $("#auto_tab_main1 .auto_tab_context div[var='1']").html(html);
            var width = $(window).width();
            var height = $(window).height();
            $("#auto_tab_main1").animate({left: ((width - 450) / 2) + "px", top: ((height - 540) / 2) + "px"}, 500);
        });
    });
    ...
    $("#add_physica_form input").keyup(function () {
        var that = $(this);
        var val = that.val();
        if (val.indexOf(" ") !== -1) {
    ...
    //只有指定的管理员能补录数据
    ycoa.ajaxLoadGet("/api/second_sale_soft/physica.php", {action: 21}, function (result) {
        is_manager = result.is_manager;
        if (result.is_manager) {
            $("#add_physica_form .date-time-picker-bind-mouseover").datetimepicker({autoclose: true});
            $("#add_fillarrears_form .date-time-picker-bind-mouseover").datetimepicker({autoclose: true});
            $("#add_physica_form #customer,#add_fillarrears_form #customer").click(function () {
            ...
            $("#add_physica_form #add_time_manager").remove();
    ...
    $("#agent_category").change(function(){
        ...
        if($("#agent_category_selected").text() == "请在下面选择类目"){
            $("#agent_category_selected").text("");
        }
        if($("#agent_category_selected").text().indexOf($(this).val())!=-1){
           // alert("请不要重复选择");
         
          var agent_category_text = $("#agent_category_selected").text();
          var replace_text = $(this).val().toString();
          agent_category_text= agent_category_text.replace(replace_text,"");
          $("#agent_category_selected").text("");
          $("#agent_category_selected").text(agent_category_text);
        }
         else{
             $("#agent_category_selected").append($(this).val()+" ");
         }
