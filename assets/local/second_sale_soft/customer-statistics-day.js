...
var CustomerStatisticsDayViewModel = new function () {
    ...
    self_.doEditCustomerStatistics = function (customerStatistics) {
      //正则验证只能输入正整数和0
        ...
        ycoa.ajaxLoadPost("/api/second_sale_soft/customer_statistics.php", JSON.stringify(customerStatistics), function (result) {
            if (result.code == 0) {
   ...
   //导出EXCEL
    ...
    $("#btn_toexcel_primary").live("click", function () {
        ...
        if (start_time || end_time) {
            location.href = "/api/second_sale_soft/customer_statistics.php?start_time=" + start_time + "&end_time=" + end_time + "&action=11";
        }
    });
    $("#show_ranking").click(function () {
        ycoa.ajaxLoadGet("/api/second_sale_soft/customer_statistics.php", {action:13,time_unit:1,team:1}, function (result) {
            var html = "<ul>";
             html += "<li><span class='num'>排名</span><span class='presales' style='width:120px;'>姓名</span><span class='count'style='width:100px;'>二销金额</span><span class='money'>换算金额</span></li>";
...
//                var v = self.attr("var");
//                ycoa.ajaxLoadGet("/api/second_sale_soft/customer_statistics.php", {action: 13, time_unit:1,team:v}, function (result) {
//                    var html = "<ul>";
//                    html += "<li><span class='num'>排名</span><span class='presales' style='width:100px;'>姓名</span><span class='count'style='width:100px;'>二销金额</span><span class='money'>换算金额</span></li>";
//                    $.each(result, function (index, d) {
//                       
//                        html += "<li><span class='num'>" + (index + 1) + "</span><span class='presales' style='width:100px;'>" + (d.user_name) + "</span><span class='count' style='width:100px;'>" + (d.sum) + "</span><span class='money'>" + (d.proportion)+ "</span></li>";
//                    });
//                    html += "</ul>";
