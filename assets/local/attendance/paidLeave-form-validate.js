$('#add_paidLeave_form').validate({
    ...
    submitHandler: function (form) {
        var data = $("#add_paidLeave_form").serializeJson();
        data.action = 1;
        data = JSON.stringify(data);
        ycoa.ajaxLoadPost("/api/attendance/paidLeave.php", data, function (result) {
            if (result.code == 0) {
                $("#btn_close_primary").click();
                paidLeaveListViewModel.listPaidLeave({});
