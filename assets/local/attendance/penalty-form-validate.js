$('#add_penalty_form').validate({
    ...
    submitHandler: function (form) {
        var data = $("#add_penalty_form").serializeJson();
        data.action = 1;
        data = JSON.stringify(data);
        ycoa.ajaxLoadPost("/api/attendance/penalty.php", data, function (result) {
            if (result.code == 0) {
                $("#btn_close_primary").click();
                penaltyListViewModel.listPenalty({});
