$('#add_absenteeism_form').validate({
    ...
    submitHandler: function (form) {
        var data = $("#add_absenteeism_form").serializeJson();
        data.action = 1;
        data = JSON.stringify(data);
        ycoa.ajaxLoadPost("/api/attendance/absenteeism.php", data, function (result) {
            if (result.code == 0) {
                $("#btn_close_primary").click();
                absenteeismListViewModel.listAbsenteeism({});
