    ...
    self_.listPlatform = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale/platform.php", data, function (results) {
            ...
            $.each(results.list, function (index, platform) {
                ...
                platform.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("3050102") && ( ycoa.user.userid() === platform.customer_id || platform.is_manager === true);;
                platform.edit = ycoa.SESSION.PERMIT.hasPagePermitButton("3050103") && ( (ycoa.user.userid() === platform.customer_id && platform.is_edit === 0) || platform.is_manager === true);
                platform.show = ycoa.SESSION.PERMIT.hasPagePermitButton("3050104");
                platform.te = ycoa.SESSION.PERMIT.hasPagePermitButton('3050106') && platform.isTe === 0;
...
$(function () {
    ...
    $("#open_dialog_btn").click(function () {
        ...
        //只有指定的管理员能补录数据
        ycoa.ajaxLoadGet("/api/second_sale/platform.php", {action:21}, function (result) {
            if(result.is_manager){
                $("#add_platform_form #add_time_manager").show();
                $("#add_platform_form #customer").val("");
                 $("#add_platform_form #customer").live("click", function () {
                    ycoa.UI.empSeleter({el: $(this), type: 'only', groupId: [6]}, function (data, el) {
                        el.val(data.name);
                        $("#add_platform_form #customer_id").val(data.id);
                    });
                });
            }
            else{
    ...
    $("#add_platform_form #quick_write").keypress(function (e) {
        if (e.keyCode === 13 && $(this).val()) {
            $.get(ycoa.getNoCacheUrl("/api/sale/salecount.php"), {action: 20, qk: $(this).val()}, function (res) {
                if (res.length === 1) {
                    var data = res[0];
                    $("#add_platform_form input").each(function () {
                        var el = $(this);
...
var array = {
    ...
    payment_method: function () {
        var val = "llw15888@163.com(李云娇),xiaojie13141an@qq.com(冯杰),左右女鞋店主,llw52020@163.com(孙豪),llw8988@163.com(周峰),zhouslowly@foxmail.com(周峰),286878263@qq.com(周峰),kllpay@126.com(王远敏),kellpay@163.com(王远敏),lingfanllvip@163.com(刘博),中国银行(王远敏),邮政银行(王远敏),交通银行(王远敏),工商银行(王远敏),建设银行(王远敏),农业银行(王远敏),农村信用社(王远敏),店铺/掌柜：飞凰品牌,微信llw8988(王远敏),微信:fsxwxsk(王远敏),微信:fsxskzy(王远敏),财付通:2405658931(王远敏)";
        val = val.split(",");
        var array = new Array();
