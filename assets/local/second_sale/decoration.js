    ...
    self_.listDecoration = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale/decoration.php", data, function (results) {
            ...
            $.each(results.list, function (index, decoration) {
                ...
                decoration.dele = ycoa.SESSION.PERMIT.hasPagePermitButton('3050302')  && ( ycoa.user.userid() === decoration.customer_id || decoration.is_manager === true);
                decoration.edit = ycoa.SESSION.PERMIT.hasPagePermitButton('3050303') && ( (decoration.isTe === 0 && ycoa.user.userid() === decoration.customer_id && decoration.is_edit === 0) || decoration.is_manager === true);//售后一般人员在审核后没有修改权限;
                decoration.show = ycoa.SESSION.PERMIT.hasPagePermitButton('3050304');
                decoration.te = ycoa.SESSION.PERMIT.hasPagePermitButton('3050307') && decoration.isTe === 0 && decoration.is_manager === true;
...
$(function () {
    ...
    $("#open_dialog_btn").click(function () {
     ...
     //   $("#add_decoration_form #customer").val(ycoa.user.username());
       ...
       //只有指定的管理员能补录数据
      ycoa.ajaxLoadGet("/api/second_sale/decoration.php", {action:21}, function (result) {
          if(result.is_manager){
              $("#add_decoration_form #add_time_manager").show();
              $("#add_decoration_form #customer").val("");
              $("#add_decoration_form #customer").live("click", function () {
                  ycoa.UI.empSeleter({el: $(this), type: 'only', groupId: [6]}, function (data, el) {
                      el.val(data.name);
                      $("#add_decoration_form #customer_id").val(data.id);
                  });
              });
          }
          else{
...
var array = {
    ...
    decoration_packages: [{id: '单次装修', text: '单次装修'}, {id: 'VIP1', text: 'VIP1'}, {id: 'VIP2', text: 'VIP2'}, {id: 'VIP3', text: 'VIP3'}, {id: 'VIP4', text: 'VIP4'}],
