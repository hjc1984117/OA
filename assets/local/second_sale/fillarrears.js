var FillarrearsListViewModel = new function () {
    ...
    self_.listFillarrears = function (data) {
        ycoa.ajaxLoadGet("/api/second_sale/fillarrears.php", data, function (results) {
            ...
            $.each(results.list, function (index, fillarrears) {
                fillarrears.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("3050602") && ( ycoa.user.userid() === fillarrears.customer_id || fillarrears.is_manager === true);
                fillarrears.edit = function () {//售后一般人员在审核后没有修改权限;
                    if (fillarrears.type === 1) {
                        return ycoa.SESSION.PERMIT.hasPagePermitButton("3050603") && ( (ycoa.user.userid() === fillarrears.customer_id && fillarrears.is_edit === 0) || fillarrears.is_manager === true);
                    } else {
                        return ycoa.SESSION.PERMIT.hasPagePermitButton("3050603")  && ( (fillarrears.isTe === 0 && ycoa.user.userid() === fillarrears.customer_id && fillarrears.is_edit === 0) || fillarrears.is_manager === true);
                    }
                }();
                fillarrears.show = ycoa.SESSION.PERMIT.hasPagePermitButton("3050604");
                fillarrears.te = ycoa.SESSION.PERMIT.hasPagePermitButton('3050606') && fillarrears.isTe === 0 && fillarrears.is_manager && fillarrears.type > 1;
    ...
    self_.delFillarrears = function (fillarrears) {
        ycoa.UI.messageBox.confirm('确认删除？', function (del) {
            if (del) {
                ...
                ycoa.ajaxLoadPost("/api/second_sale/fillarrears.php", JSON.stringify(fillarrears), function (result) {
                    if (result.code == 0) {
...
$(function () {
    ...
    $("#btn_toexcel_primary").live("click", function () {
        ...
        if (start_time || end_time) {
            location.href = '/api/second_sale/fillarrears.php?start_time=' + start_time + '&end_time=' + end_time + '&action=11';
        }
    });
    $(".dept_submit_btn").live("click", function () {
        ...
        ycoa.ajaxLoadPost("/api/second_sale/fillarrears.php", data, function (result) {
            if (result.code == 0) {
                ycoa.UI.toast.success(result.msg);
                $('.cancel_btn').click();
                reLoadData({});
...
function initEditSeleter(el, type) {
    ...
    $("#customer", el).click(function () {
        ycoa.UI.empSeleter({el: $(this), type: 'only', groupId: [6]}, function (d, el) {
            el.val(d.name);
            el.parent().find("#customer_id").val(d.id);
        });
    });
}
var array = {
