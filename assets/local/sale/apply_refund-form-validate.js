$('#add_apply_refund_form').validate({
    ...
    unhighlight: function (element) { // revert the change done by hightlight
        ...
        $(element).closest('.col-md-10').removeClass('has-error'); // set error class to the control group
    },
    success: function (label, element) {
        ...
        $(element).closest('.col-md-10').removeClass('has-error').addClass('has-success'); // set success class to the control group
        icon.removeClass("fa-warning").addClass("fa-check");
    },
    submitHandler: function (form) {
        var data = $("#add_apply_refund_form").serializeJson();
        data.action = 1;
        ycoa.ajaxLoadPost("/api/sale/apply_refund.php", JSON.stringify(data), function (result) {
            if (result.code === 0) {
                $("#btn_pply_refund_close_primary").click();
