    ...
    self_.listAccess = function (data) {
        ycoa.ajaxLoadGet("/api/sale/qq_access.php", data, function (results) {
            ...
            $.each(results.list, function (index, access) {
                access.dele = ycoa.SESSION.PERMIT.hasPagePermitButton("2011002") && access.add_userid !== ycoa.user.userid();
                access.edit = ycoa.SESSION.PERMIT.hasPagePermitButton("2011003");// && access.add_userid == ycoa.user.userid();
                access.show = ycoa.SESSION.PERMIT.hasPagePermitButton("2011004");
                access.can_access = ycoa.SESSION.PERMIT.hasPagePermitButton("2011005") && ((!access.access_time) ? true : false) && access.presales_id === ycoa.user.userid();
...
$(function () {
    ...
    $("#dataTable").searchUserName(function (name) {
        reLoadData({
            action: 1,
            sort: ycoa.SESSION.SORT.getSort(), sortname: ycoa.SESSION.SORT.getSortName(),
            searchName: name, searchTime: $('#searchDateTime').val(), searchChannel: $("#searchChannel").val()
        });
    }, '按关键字搜索');
    $("#dataTable").searchAutoStatus(channelSearchArray, function (d) {
        reLoadData({
            action: 1,
            sort: ycoa.SESSION.SORT.getSort(), sortname: ycoa.SESSION.SORT.getSortName(),
            searchName: $("#searchUserName").val(), searchTime: $('#searchDateTime').val(), searchChannel: d.id
        });
    }, "渠道搜索", "searchChannel");
    $("#dataTable").searchDateTime(function (d) {
        reLoadData({
            action: 1,
            sort: ycoa.SESSION.SORT.getSort(), sortname: ycoa.SESSION.SORT.getSortName(),
            searchName: $("#searchUserName").val(), searchTime: d
        });
    }, '添加时间');
    ...
    $("#btn_toexcel_primary").live("click", function () {
        ...
        if (start_time || end_time) {
            location.href = '/api/sale/qq_access.php?start_time=' + start_time + '&end_time=' + end_time + '&action=11';
        }
    });
...
function initEditSeleter(el) {
    $("#presales", el).click(function () {
        ycoa.UI.empSeleter({el: $(this), type: 'only', groupId: [7]}, function (d, el) {
            el.val(d.name);
...
var channelArray = [{id: '百度(PC)', text: '百度(PC)', default: true}, {id: '百度(移动)', text: '百度(移动)'}, {id: '360', text: '360'}, {id: '搜狗', text: '搜狗'}];
var channelSearchArray = [{id: '', text: '全部'}, {id: '百度(PC)', text: '百度(PC)', default: true}, {id: '百度(移动)', text: '百度(移动)'}, {id: '360', text: '360'}, {id: '搜狗', text: '搜狗'}];