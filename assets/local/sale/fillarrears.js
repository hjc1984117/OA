...
//        $("#customer_id2").val(fillarrears.customer_id);
        $("#customer_id").val(fillarrears.customer_id);
...
//        var nick_name = t[1];
//        var customer = t[0];
//        updateCL({id: id, status: status, customer_id: customer_id, nick_name: nick_name, customer: customer});
            ...
            if (thisVal && thisVal.length >= 2) {
                $.get(ycoa.getNoCacheUrl("/api/sale/salecount.php"), {action: 3, key_word: thisVal}, function (res) {
                    if (res.length == 1) {
                        ...
                        $("#add_fill_form #remark_fillarrears").html("");
                    }
                });
            } else {
