$('#add_sale_statistics_day_form').validate({
    ...
    submitHandler: function (form) {
        ycoa.UI.messageBox.confirm("<span style='color:red;font-size:20px;'>确认提交?提交后,该数据将不能修改,请慎重考虑~<br>错了会罚款的噢 ~  亲亲~</span>", function (btn) {
            if (btn) {
                var data = $("#add_sale_statistics_day_form").serializeJson();
                data.action = 1;
                //data.presales = ycoa.user.userid();
                ycoa.ajaxLoadPost("/api/sale/saleStatisticsDay.php", JSON.stringify(data), function (result) {
                    if (result.code == 0) {
                        $("#btn_close_primary").click();
                        ycoa.UI.toast.success(result.msg);
                        SaleStatisticsDayViewModel.listSaleStatisticsDay({action: 1});
                    } else {
