...
if (glob($configManager->getConfig('path.swf') . $doc . "*") != false) $pagecount = count(glob($configManager->getConfig('path.swf') . $doc . "*"));
else $pagecount = 0;
...
if ($pagecount == 0 && validPdfParams($pdfFilePath, $doc, $page)) {
    ...
    if (rtrim($output) === "[Converted]") $pagecount = count(glob($configManager->getConfig('path.swf') . $doc . "*"));
}else {
