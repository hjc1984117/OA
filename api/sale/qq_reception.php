...
execute_request(HttpRequestMethod::Get, function() use($action) {
    ...
    if (isset($sort) && isset($sortname)) {
        ...
        $sql .= "ORDER BY pr.status0 ASC,pr.status ASC";
    }
    $db = create_pdo();
...
execute_request(HttpRequestMethod::Post, function() use($action) {
    ...
    if ($action == 4) {
        $db = create_pdo();
        $sql = "update P_QQReception SET status = 1 where status0 = 1";
    ...
    if ($action == 30) {
        $db = create_pdo();
        $sql = "update P_QQReception SET status0 = " . $qq_receptionData->status0;
        ...
        pdo_transaction($db, function($db) use($sql, $qq_receptionData) {
            add_data_change_log($db, $qq_receptionData, new p_qqreception($qq_receptionData->id), 10001, (($qq_receptionData->status0) === 0 ? "下班" : "上班"));
            $result = Model::execute_custom_sql($db, $sql);
            if (!$result[0]) throw new TransactionException(PDO_ERROR_CODE, '启动失败~' . $result['detail_cn'], $result);
        });
    ...
    if ($action == 40) {
        $db = create_pdo();
        $sql = "update P_QQReception SET status = " . $qq_receptionData->status . " where id = " . $qq_receptionData->id;
        pdo_transaction($db, function($db) use($sql, $qq_receptionData) {
            add_data_change_log($db, $qq_receptionData, new p_qqreception($qq_receptionData->id), 10000, (($qq_receptionData->status) === 0 ? "暂停" : "启用"));
            $result = Model::execute_custom_sql($db, $sql);
            if (!$result[0]) throw new TransactionException(PDO_ERROR_CODE, '启动失败~' . $result['detail_cn'], $result);
        });
