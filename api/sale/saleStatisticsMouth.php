...
execute_request(HttpRequestMethod::Get, function() use($action) {
    if ($action == 1) {
        ...
        $manager_role_ids = array(0, 102, 408, 701, 702, 703, 704, 709, 710,);
        ...
        $sql = "SELECT s.userid,s.username,s.channel,s.salecount_lv,SUM(s.commission) AS commission,SUM(s.into_count) AS into_count,SUM(s.accept_count) AS accept_count,SUM(s.deal_count) AS deal_count,";
        $sql.="SUM(s.timely_count) AS timely_count,SUM(s.amount) AS amount,DATE_FORMAT(s.reltime,'%Y-%m') AS reltime FROM p_salestatistics s WHERE 1=1 ";
        if (isset($searchName)) {
        ...
        if ($is_manager) {
            $month_count_sql = "SELECT ps.salecount_lv, IFNULL(SUM(ps.into_count),0) AS into_count,IFNULL(SUM(ps.accept_count),0) AS accept_count,IFNULL(SUM(ps.commission),0) AS commission,IFNULL(SUM(ps.deal_count),0) AS deal_count,IFNULL(SUM(ps.timely_count),0) AS timely_count,IFNULL(SUM(ps.amount),0) AS amount,IFNULL(DATE_FORMAT(ps.reltime,'%Y-%m'),'" . $searchTime . "') AS reltime from P_SaleStatistics ps ";
    ...
    if ($action == 11) {
        ...
        $field = array('username', 'channel', 'into_count', 'accept_count', 'deal_count', 'timely_count', 'amount', 'commission', 'loss_number', 'loss_rate', 'timely_rate', 'timely_turnover_ratio', 'conversion_rate', 'average_price', 'reltime');
