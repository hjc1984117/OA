...
execute_request(HttpRequestMethod::Get, function() use($action) {
    ...
    $manager_userids = array(43, 187, 39, 48, 42, 52, 291, 298, 324, 326, 349, 16, 441);
    ...
    if ($action == 1) {
        ...
        $models = Model::list_to_array($result['models'], array(), function(&$d) use($is_manager, $login_userid) {
                    if (!$is_manager && !str_equals($d['rception_staff_id'], $login_userid) && !str_equals($d['customer_id'], $login_userid)) {
                        $d['ww'] = '****';
                        $d['name'] = '***';
                        $d['qq'] = '********';
                        $d['alipay_account'] = '***********';
    ...
    if ($action == 5) {
        $platform = new p_platform();
        $platform->set_query_fields(array('id', 'ww', 'qq', 'money', 'phone', 'payment_method'));
    ...
    if ($action == 6) {
        ...
        if (isset($search_name)) {
            $platform = new p_platform();
            $platform->set_custom_where(" AND ( ww = '" . $search_name . "' OR alipay_account = '" . $search_name . "' OR name = '" . $search_name . "' OR qq = '" . $search_name . "' ) ");
    ...
    if ($action == 10) {
        ...
        $fields = array('add_time', 'ww', 'qq', 'name', 'money', 'diamond_card', 'p_arrears', 'alipay_account', 'payment_method', 'customer', 'rception_staff', 'isTeach', 'md_arrears', 'tra_num');
        $platform->set_query_fields($fields);
        $db = create_pdo();
        $result = Model::query_list($db, $platform, NULL, true);
        if (!$result[0]) {
        ...
        $models = Model::list_to_array($result['models'], array(), function(&$d) {
            ...
            $fields = array('add_time', 'ww', 'qq', 'name','showname', 'money', 'diamond_card', 'p_arrears', 'alipay_account', 'payment_method', 'customer', 'rception_staff', 'isTeach', 'md_arrears', 'tra_num');
            
        $expolt->set_field($fields);
        $title_array = array('添加时间', '旺旺', 'QQ', '真实姓名', '支付对应姓名', '付款金额', '钻卡', '平台欠款', '支付宝账号', '支付方式', '售后名称', '平台接待人员', '是否教学', '实物/装修欠款', '交易号');
    ...
    if ($action == 20) {
        ...
        $platform->set_query_fields(array('ww', 'qq', 'phone', 'money')); //'name',
        $db = create_pdo();
        $result = Model::query_list($db, $platform);
        if (!$result[0]) die_error(USER_ERROR, '获取统计资料失败，请重试');
