...
execute_request(HttpRequestMethod::Get, function() use($action) {
    ...
    $manger_user_ids =array(35,37,189,422);//目前除了超级管理员只允许一灯，孔明和夏娜能查看所有售后的统计记录。其他售后只能看自己的记录，其他部门和角色的都不能看售后统计.为了调试方便，暂时加上422
    ...
    if($action == 1){
        ...
        if(isset($searchTeam)){
            $customerstatistics->set_where_and(p_customerstatistics::$field_team, SqlOperator::Equals, $searchTeam);
        }
        if(isset($searchGroup)){
            $customerstatistics->set_where_and(p_customerstatistics::$field_group, SqlOperator::Equals, $searchGroup);
        }
        if (isset($searchName)) {
            $customerstatistics->set_where_and(p_customerstatistics::$field_user_name, SqlOperator::Like, '%'.$searchName.'%');
        }
        if (!$is_manager) {
        ...
        $customerstatistics->set_order_by(p_customerstatistics::$field_date, SqlSortType::Desc);
    ...
    if ($action == 11) {
        ...
        $customerStatistics->set_order_by(p_customerstatistics::$field_date, 'desc');
        $db = create_pdo();
    ...
    if($action ==13){
           ...
           switch($time_unit){
               case 1:
               default:                  
                   $sql = 'SELECT pc.user_name,pc.group,pc.flow_num+pc.physical_things+pc.decorate AS sum FROM p_customerstatistics AS pc LEFT JOIN m_user AS mu ON pc.user_id=mu.userid WHERE mu.`status` =1';
...
//                          continue;
               ...
               case 2: 
                    ...
                    FROM p_customerstatistics AS s LEFT JOIN m_user AS mu ON s.user_id=mu.userid WHERE s.group=2 ".$date_sql."  AND mu.`status` =1 AND s.team=".$team." GROUP BY s.group_members)
                    UNION ALL
                    (SELECT 
                    s.user_name,s.group,SUM(s.have_received) AS have_received,SUM(s.flow_num)+SUM(s.physical_things)+SUM(s.decorate) AS second_sales,
                    SUM(s.singular) AS singular,date 
                    FROM p_customerstatistics AS s LEFT JOIN m_user AS mu ON s.user_id=mu.userid WHERE s.group=1 ".$date_sql." AND mu.`status` =1 AND s.team=".$team." GROUP BY s.user_id)) AS u 
...
execute_action(HttpRequestMethod::Post, 1, function() {
    ...
    $customerstatistics->set_where_and(p_customerstatistics::$field_id, SqlOperator::Equals, $customerStatisticsData->id);
    $db = create_pdo();
...
function get_title_statistics_data($searchTime=NULL){ 
        $db = create_pdo();
        if(!$searchTime){
             $searchTime = date("Y-m-d");      
            if(date("H")<3){
        ...
        $sql="SELECT SUM(platform.money) AS platform_money,COUNT(0) AS platform_count,customerrecord.team FROM p_platform AS platform LEFT JOIN p_customerrecord AS customerrecord ON platform.customer_id = customerrecord.userid WHERE DATE_FORMAT(add_time,'%Y-%m-%d %H:%i:%s') >= '" . $searchStartTime . "' AND DATE_FORMAT(add_time,'%Y-%m-%d %H:%i:%s') <= '" . $searchEndTime . "' GROUP BY customerrecord.team";
        ...
        $sql="SELECT SUM(physica.agent_price) AS physica_money,COUNT(0) AS physica_count,customerrecord.team FROM p_physica AS physica LEFT JOIN p_customerrecord AS customerrecord ON physica.customer_id = customerrecord.userid WHERE isTe = 1 AND DATE_FORMAT(add_time,'%Y-%m-%d %H:%i:%s') >= '" . $searchStartTime . "' AND DATE_FORMAT(add_time,'%Y-%m-%d %H:%i:%s') <= '" . $searchEndTime . "' GROUP BY customerrecord.team";
        $result = Model::execute($db, $sql);  
        $result = $result['results'];
          foreach($result as $data){
        ...
        $sql="SELECT SUM(decoration.decoration_price) AS decoration_money ,COUNT(0) AS decoration_count,customerrecord.team FROM p_decoration AS decoration LEFT JOIN p_customerrecord AS customerrecord ON decoration.customer_id = customerrecord.userid WHERE isTe = 1 AND DATE_FORMAT(add_time,'%Y-%m-%d %H:%i:%s') >= '" . $searchStartTime . "' AND DATE_FORMAT(add_time,'%Y-%m-%d %H:%i:%s') <= '" . $searchEndTime . "' GROUP BY customerrecord.team";
        $result = Model::execute($db, $sql);  
        $result = $result['results'];
        foreach($result as $data){
...
SUM(s.singular) AS singular,date 
FROM p_customerstatistics as s WHERE s.group=2 ".$sql." GROUP BY s.group_members)
UNION ALL
(SELECT 
s.id,s.user_id,s.suser_id,s.user_name,s.team,s.group,s.group_members,s.category,SUM(s.have_received) AS have_received,SUM(s.receive_amount) AS receive_amount,SUM(s.refund_num) AS refund_num,
SUM(s.refund_amount) AS refund_amount,SUM(s.change_Q) AS change_Q,SUM(s.flow_num) AS flow_num,SUM(s.physical_things) AS physical_things,SUM(s.decorate) AS decorate,
SUM(s.singular) AS singular,date 
FROM p_customerstatistics AS s WHERE s.group=1 ".$sql." GROUP BY s.user_id)) AS u where 1=1 ";
}
...
//             if(($data_array_first['user_id'] == $data_array_second['suser_id']) && ($data_array_second['group'] == 2)){
//                 
