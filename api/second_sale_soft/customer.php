...
execute_request(HttpRequestMethod::Get, function() use($action) {
    ...
    if ($action == 1) {
        ...
        if (isset($searchName)) {
            $customer->set_where_and(P_Customerrecord_second_soft::$field_username, SqlOperator::Like, "%" . $searchName . "%");
        }
        if (isset($type)) {
            $customer->set_where_and(P_Customerrecord_second_soft::$field_type, SqlOperator::Equals, $type);
        }
        if (isset($sort) && isset($sortname)) {
            $customer->set_order_by($customer->get_field_by_name($sortname), $sort);
        } else {
            $customer->set_order_by(P_Customerrecord_second_soft::$field_id, 'ASC');
        }
    ...
    if ($action == 2) {
        $customer = new P_Customerrecord_second_soft();
        $customer->set_where_and(P_Customerrecord_second_soft::$field_status, SqlOperator::Equals, 1);
        if (isset($type)) {
            $customer->set_where_and(P_Customerrecord_second_soft::$field_type, SqlOperator::Equals, $type);
        }
...
execute_request(HttpRequestMethod::Post, function() use($action) {
    ...
    if ($action == 5) {
        $customer = new P_Customerrecord_second_soft($customerDate->id);
        $customer->set_field_from_array($customerData);
    ...
    if ($action == 4) {
        $db = create_pdo();
        $sql = "update P_Customerrecord_second_soft SET status = 1";
    ...
    if ($action == 41) {
        $db = create_pdo();
        $sql = "update P_Customerrecord_second_soft SET status = 0";
        pdo_transaction($db, function($db) use($sql) {
            $result = Model::execute_custom_sql($db, $sql);
