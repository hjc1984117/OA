...
execute_request(HttpRequestMethod::Get, function() use($action) {
    if ($action == 1) {
        ...
        if (isset($searchName)) {
            $saleStatistics->set_where_and(p_salestatistics_soft::$field_username, SqlOperator::Like, '%' . $searchName . '%');
        }
        if (isset($searchTime)) {
            $saleStatistics->set_custom_where(" AND DATE_FORMAT(reltime,'%Y-%m-%d') = '" . $searchTime . "' ");
        }
        if (!$is_manager) {
            $saleStatistics->set_where_and(p_salestatistics_soft::$field_userid, SqlOperator::Equals, request_login_userid());
        }
        if (isset($searchChannel)) {
            $saleStatistics->set_where_and(p_salestatistics_soft::$field_channel, SqlOperator::Equals, $searchChannel);
        }
        if (isset($sort) && isset($sortname)) {
            $saleStatistics->set_order_by($saleStatistics->get_field_by_name($sortname), $sort);
        } else {
            $saleStatistics->set_order_by(p_salestatistics_soft::$field_reltime, 'desc');
        }
        ...
        $models = Model::list_to_array($result['models'], array(), function(&$d) use($is_manager, $userid, $h) {
        ...
        $d = array('into_count' => 0, 'accept_count' => 0, 'deal_count' => 0, 'timely_count' => 0, 'amount' => 0.00, 'commission' => '/', 'loss_number' => 0, 'loss_rate' => 0, 'elderly_deal_count' => 0, 'timely_rate' => 0, 'timely_turnover_ratio' => 0, 'conversion_rate' => 0, 'average_price' => 0, 'reltime' => $searchTime);
        if ($is_manager) {
            $month_count_sql = "SELECT IFNULL(SUM(ps.into_count),0) AS into_count,IFNULL(SUM(ps.elderly_deal_count),0) AS elderly_deal_count,IFNULL(SUM(ps.accept_count),0) AS accept_count,IFNULL(SUM(ps.deal_count),0) AS deal_count,IFNULL(SUM(ps.commission),0) AS commission,";
            $month_count_sql.="IFNULL(SUM(ps.timely_count),0) AS timely_count,IFNULL(SUM(ps.amount),0) AS amount,'" . $searchTime . "' AS reltime from p_salestatistics_soft ps WHERE 1=1 ";
    ...
    if ($action == 11) {
        ...
        $title_array = array('姓名', '提成等级', '渠道', '转入数/日', '接入数/日', '成交数/日', '及时数/日', '金额/日', '提成/日', '老人成交数/日', '流失数/日', '流失率/日', '及时率/日', '及时成交占比', '转化率/日', '均价/日', '日期');
        $field = array('username', 'salecount_lv', 'channel', 'into_count', 'accept_count', 'deal_count', 'elderly_deal_count', 'timely_count', 'amount', 'commission', 'loss_number', 'loss_rate', 'timely_rate', 'timely_turnover_ratio', 'conversion_rate', 'average_price', 'reltime');
...
execute_request(HttpRequestMethod::Post, function() use($action) {
    ...
    if ($action == 1) {
        ...
        $saleStatistics = new p_salestatistics_soft();
        $saleStatistics->set_custom_where(" AND userid = " . request_login_userid() . " DATE_FORMAT(reltime,'%Y-%m-%d') ='" . $reltime . "' ");
        $db = create_pdo();
        ...
        $commission = get_soft_commission($amount, $user->get_salecount_lv());
    ...
    if ($action == 3) {
        ...
        if (!$result_user[0]) die_error(USER_ERROR, '添加统计失败~');
        $amount = (int) $saleStatisticsData->amount;
        $commission = get_soft_commission($amount, $saleStatisticsData->salecount_lv);
        $saleStatistics->set_commission($commission);
